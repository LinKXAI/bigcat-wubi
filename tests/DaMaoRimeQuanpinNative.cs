...
public static class DaMaoRimeQuanpinNative
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate int GetOptionDelegate(UIntPtr session, [MarshalAs(UnmanagedType.LPStr)] string name);
    ...
    public static ulong CreateDefaultSession() { return createSession().ToUInt64(); }
    public static string CurrentSchema(ulong session) {
        StringBuilder buffer = new StringBuilder(256);
        if(Function<GetSchemaDelegate>(33)(new UIntPtr(session), buffer, new UIntPtr(256)) == 0) throw new Exception("No current schema");
        return buffer.ToString();
    }
    public static bool Option(ulong session, string name) { return Function<GetOptionDelegate>(28)(new UIntPtr(session), name) != 0; }
    public static bool Select(ulong session, string name) { return selectSchema(new UIntPtr(session), name) != 0; }
    public static void Clear(ulong session) { clearComposition(new UIntPtr(session)); }
    public static string Commit(ulong session) { return ReadCommit(new UIntPtr(session)); }
    public static string[] View(ulong session) {
        ...
        try {
            ...
            values.Add(Utf8(c.composition.preedit)); values.Add(c.menu.page.ToString());
            for(int i=0;i<c.menu.count;i++) {
                RimeCandidate candidate=(RimeCandidate)Marshal.PtrToStructure(IntPtr.Add(c.menu.candidates,i*Marshal.SizeOf(typeof(RimeCandidate))),typeof(RimeCandidate));
                values.Add(Utf8(candidate.text));
            }
