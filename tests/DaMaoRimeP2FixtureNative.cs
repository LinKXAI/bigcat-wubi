...
// Test-only real-librime fixture builder. Production backup uses the narrower
// DaMaoRimeLeversAdapter and never exposes session or delete operations.
public static class DaMaoRimeP2FixtureNative
{
    ...
    private static T Function<T>(int index) where T : class
    {
        ...
        if (sizeof(int) + dataSize <= offset)
            throw new InvalidOperationException("Fixture API boundary failure at index " + index + ".");
        IntPtr address = Marshal.ReadIntPtr(api, offset);
        if (address == IntPtr.Zero)
            throw new InvalidOperationException("Fixture API function unavailable at index " + index + ".");
    ...
    public static void Load(string dllPath, string sharedDataDir, string userDataDir, string stagingDir)
    {
        ...
        if (module == IntPtr.Zero) throw new InvalidOperationException("Cannot load fixture rime.dll.");
        IntPtr entry = GetProcAddress(module, "rime_get_api");
        if (entry == IntPtr.Zero) throw new InvalidOperationException("Fixture rime_get_api unavailable.");
        ...
        traits.distribution_name = "DaMao P2 Fixture";
        traits.distribution_code_name = "DaMaoP2Fixture";
        traits.distribution_version = "2";
        traits.app_name = "rime.damao_p2_fixture";
    ...
    private static int CandidateCount(UIntPtr session)
    {
        RimeCandidateListIterator iterator = new RimeCandidateListIterator();
        if (candidateBegin(session, ref iterator) == 0) return 0;
        int count = 0;
        try { while (candidateNext(ref iterator) != 0) count++; }
        finally { candidateEnd(ref iterator); }
        return count;
    }

    public static string CommitFirst(ulong value, string input)
    {
        UIntPtr session = new UIntPtr(value);
        if (setInput(session, input) == 0 || CandidateCount(session) == 0 ||
            selectCandidate(session, UIntPtr.Zero) == 0 || commitComposition(session) == 0)
    ...
    public static bool DeleteFirst(ulong value, string input)
    {
        UIntPtr session = new UIntPtr(value);
        return setInput(session, input) != 0 && CandidateCount(session) > 0 &&
            deleteCandidate(session, UIntPtr.Zero) != 0;
    }

    public static void CloseSession(ulong value) { destroySession(new UIntPtr(value)); }
